                                        ...
                                        <tr class="invoice-row">
                                            <td style="width: 280px;">
                                                <div class="position-relative">
                                                    <div>
                                                        ...
                                                        @if($invoiceDetails->status == ACTIVE_STATUS || $invoiceDetails->attempt_by_admin == ACTIVE_STATUS)
                                                            <input type="text" name="rows[{{ $index }}][productName]" value="{{$item->product->name.' '.$item->product->strength.' '.\Illuminate\Support\Str::limit($item->product->type,3,'') .' - '.\Illuminate\Support\Str::limit($item->product->company->name,3,''),}}" class="form-control" placeholder="Product Name" readonly style="width: 100%">
                                                        @else
                                                            <select id="companySelect{{$index}}" class="js-example-basic-single3 form-select {{$item->product->name ? 'pass':''}}"
                                                                    style="width: 100%;">
                                                                <option value="">{{$item->product->name.' '.$item->product->strength.' '.\Illuminate\Support\Str::limit($item->product->type,3,'') .' - '.\Illuminate\Support\Str::limit($item->product->company->name,3,''),}}</option>
                                                                <!-- options go here -->
                                                            </select>
                                                        @endif
                                                    </div>
                                                ...
                                                <!-- Hidden inputs -->
                                                <input value="{{$item->id}}" name="rows[{{ $index }}][orderItemId]" type="hidden" class="form-control">
                                                <input value="{{$item->company_id}}" name="rows[{{ $index }}][companyId]" type="hidden" class="form-control">
                                                <input value="{{$item->product->name}}" name="rows[{{ $index }}][productName]" type="hidden" class="form-control">
                                                <input value="{{$item->product_id}}" name="rows[{{ $index }}][product_id]" type="hidden" class="form-control">
                                                <input value="{{$item->product->box_per_pic}}" name="rows[{{ $index }}][box_per_pic]" type="hidden" class="form-control">
                                                <input value="{{$item->product->unit_price}}" name="rows[{{ $index }}][actual_unit_price]" type="hidden" class="form-control">
                                                <input value="{{empty($item->price) ? $item->product->unit_price : $item->price}}" name="rows[{{ $index }}][unit_price]" type="hidden" class="form-control">
                                            ...
                                            <td>
                                                <div class="d-flex flex-column">
                                                    <div class="d-flex">
                                                        <input type="number" {{$invoiceDetails->status == ACTIVE_STATUS  || $invoiceDetails->attempt_by_admin == ACTIVE_STATUS ? 'readonly' :''}} name="rows[{{ $index }}][qty]" value="{{$item->box_qty ?? ''}}"
                                                               class="form-control me-2 qty-field" placeholder="Box"
                                                               style="width: 50%;">
                                                        <input type="number" {{$invoiceDetails->status == ACTIVE_STATUS || $invoiceDetails->attempt_by_admin == ACTIVE_STATUS ? 'readonly' :''}} step="any" name="rows[{{ $index }}][pieces]" value="{{$item->quantity ?? ''}}"
...
{{--                                                    <div class="text-danger validation-error mt-1 qty-pieces-error"></div>--}}
                                            ...
                                            <td>
                                                <input readonly name="rows[{{ $index }}][price]" step="any" type="number" value="{{!empty($item->price) ? $item->price : $item->product->unit_price}}" class="form-control" placeholder="Enter price">
                                            </td>

                                            <td style="display: none;">
                                                <input type="hidden" name="rows[{{ $index }}][discount]" value="{{$item->discount ?? ''}}" class="form-control" style="width: 80px">
                                            </td>

                                            {{--                                            <td>--}}
{{--                                                <input readonly name="rows[{{ $index }}][stock]" type="text" value="{{$item->product->stock ?? ''}}" class="form-control" placeholder="Stock" style="width: 80px">--}}
{{--                                            </td>--}}
{{--                                            @if (auth()->user()->role == ADMIN_ROLE)--}}
{{--                                                <td>--}}
{{--                                                    <input type="number" {{$invoiceDetails->status == ACTIVE_STATUS ? 'readonly' :''}} name="rows[{{ $index }}][discount]" value="{{$item->discount ?? ''}}" class="form-control" placeholder="Discount" step="any" style="width: 80px">--}}
{{--                                                </td>--}}
{{--                                            @endif--}}
                                            <td>
                                                <input readonly name="rows[{{ $index }}][total]" value="{{$item->final_total ?? ''}}" type="text" class="form-control" placeholder="Total" style="width: 90px">
                                            </td>
                                            <td>
                                                <button type="button" onclick="removeRow(this)" {{$invoiceDetails->status == ACTIVE_STATUS || $invoiceDetails->attempt_by_admin == ACTIVE_STATUS ? 'disabled' :''}}
                                                class="btn btn-danger btn-xs" @if (count($orderItems) === 1) disabled @endif>
                                                    <i class="fa-solid fa-delete-left"></i>
                                                </button>

{{--                                                <button onclick="addRow()" type="button" class="btn btn-primary btn-xs" id="add-row-btn"><i class="fa-solid fa-plus-circle"></i></button>--}}
                                    ...
                                    <tr>
                                        @if (auth()->user()->role == USER_ROLE)
                                            <td colspan="2">
                                                @if(($invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS) && $invoiceDetails->attempt_by_admin == PENDING_STATUS)

                                                    <button onclick="addRow()" type="button" class="btn btn-primary btn-xs mb-2" id="add-row-btn"><i class="fa-solid fa-plus-circle"></i></button>
                                                @endif
...
{{--                                                @can('apply other charge')--}}
                                                    <div class="d-flex align-items-center gap-2 mb-2">
                                                        ...
                                                        <input name="otherCharge" type="text" value="{{$otherCharge}}" readonly id="otherCharge" class="form-control text-end"
...
{{--                                                @endcan--}}
                                                ...
                                                <div class="d-flex align-items-center gap-2 mb-2">
                                                    ...
                                                    <input type="text" name="grandTotal" value="{{$grandTotal}}" id="grandTotal" class="form-control text-end grandTotal"
                                                           placeholder="0.00" readonly style="max-width: 200px;">
                                                </div>
...
{{--                                                @can('apply invoice paid amount')--}}
                                                    <div class="d-flex align-items-center gap-2 mb-2">
                                                        ...
                                                        <input type="text" id="paidAmount" value="{{$paidAmount}}"
                                                               name="paidAmount" readonly class="form-control text-end"
...
{{--                                                @endcan--}}
                            ...
                            <!-- Submit Button -->
                            @if(($invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS) && $invoiceDetails->attempt_by_admin == PENDING_STATUS)
                                <button type="submit" class="btn btn-primary submit mt-3">Update Invoice</button>
                            @endif
                            @if (auth()->user()->role == ADMIN_ROLE)
...
@push('scripts')
    ...
    <script>
        function arrowInit(){
            document.addEventListener('keydown', function(e) {
                ...
                // Check if the target is an editable input within an invoice row
                const input = e.target;
                if (input.tagName !== 'INPUT' || input.readOnly) return;
                ...
                // Get all editable inputs in the current row, excluding hidden inputs
                const inputs = Array.from(row.querySelectorAll('input:not([readonly]):not([type="hidden"])'));
                ...
                if (e.key === 'ArrowDown') {
                    ...
                    if (rowIndex + 1 < rows.length) {
                        const nextRowInputs = Array.from(rows[rowIndex + 1].querySelectorAll('input:not([readonly]):not([type="hidden"])'));
                        if (colIndex < nextRowInputs.length) {
                    ...
                    e.preventDefault();
                    if (rowIndex - 1 >= 0) {
                        const prevRowInputs = Array.from(rows[rowIndex - 1].querySelectorAll('input:not([readonly]):not([type="hidden"])'));
                        if (colIndex < prevRowInputs.length) {
        ...
        function initSelect2() {
            ...
            $el.each(function () {
                ...
                // ✅ Manually bind scroll lock even if already initialized
                $this.off('select2:open.scrollFix select2:close.scrollFix')
                    .on('select2:open.scrollFix', function () {
                        lockBodyScroll();

                        // Optional keyboard fix
                        const searchField = document.querySelector('.select2-search__field');
                        if (searchField) {
                            setTimeout(() => searchField.blur(), 50);
                        }
                    })
                    .on('select2:close.scrollFix', function () {
                        unlockBodyScroll();
                    });
                ...
                $this.on('select2:select', function (e) {
                    ...
                    row.find('input[name*="[box_per_pic]"]').val(product.box_per_pic);
                    row.find('input[name*="[unit_price]"]').val(product.unit_price);
                    row.find('input[name*="[price]"]').val(product.unit_price);
                    row.find('input[name*="[stock]"]').val(product.stock);
                });
            });
        }
        ...
        function generatePdf(type='',status=''){
            ...
            $.ajax({
                url: '{{ route('user.save-update-invoice-with-pdf') }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function calculateRowTotal(row) {
            ...
            if (totalInput) {
                ...
                totalInput.dataset.discount = customRoundFloat(discountAmount); // save for later
            }
        }

        function calculateTotalsss() {
            let totalPrice = 0;
            let totalDiscount = 0;
        ...
        function calculateAllTotals() {
            ...
            tableBody.querySelectorAll("tr").forEach(row => {
                ...
                const lineDiscount = parseFloat(totalInput?.dataset?.discount || 0);

                totalPrice += customRoundFloat(lineTotal);
            ...
            let otherCharge = (getInput("otherCharge")?.value) || 0;
            otherCharge = otherCharge.replace(/,/g, '');
            otherCharge = parseFloat(otherCharge);

            let paidAmount = (getInput("paidAmount")?.value) || 0;
        ...
        function bindRowEvents(row) {
            row.querySelectorAll("input").forEach(input => {
                input.addEventListener("input", () => {
                    calculateRowTotal(row);     // 🔁 only this row
                    calculateAllTotals();       // 🔁 recalculate totals from row data
                });
            });
        }
        ...
        const observer = new MutationObserver(() => {
            document.querySelectorAll("#invoice-order-item tr").forEach(bindRowEvents);
        });

        observer.observe(document.getElementById('invoice-order-item'), {
            childList: true,
            subtree: true
        });
    ...
    <script>
        ...
        $('#invoiceForm').on('submit', function (e) {
            ...
            $.ajax({
                url: '{{ route('user.update-invoice',$invoiceId) }}',
                type: 'POST',
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                success: function (response) {
                    ...
                    if (response.status) {
                        ...
                        // Clear any previous red borders and errors first
    ...
    <script>
        $(document).ready(function() {
            $('#pending_invoice').click(function() {
                ...
                $.ajax({
                    url: '{{ route('user.update-pending-invoice') }}',
                    type: 'POST',
                    data: formData,
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
