                ...
                <div class="card overflow-hidden">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <h3 class="mb-3 mb-md-0">{{ $page1 }}</h3>
                            <a href="" wire:click.prevent="addCustomer"
                                class="btn btn-primary btn-sm btn-icon-text"><i class="fa fa-plus-circle"></i> &nbsp;
                                Add New User</a>
                        </div>
                    </div>
                    @if (auth()->user()->role == ADMIN_ROLE || auth()->user()->role == STAFF_ROLE)
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12 col-md-6 mb-3 mb-md-0">
                                    <label for="company_id" class="form-label">Search User By Phone Or Name</label>
                                    <input class="form-control productInput" type="text"
                                        wire:model.live.debounce.300ms="query"
                                        placeholder="Enter user name or phone...">

                                    <ul class="list-group position-absolute w-100 shadow-sm suggestionList {{ empty($query) ? 'd-none' : 'd-block' }}">
                                        @foreach ($searchedUsers as $user)
                                            <li wire:key="user-{{ $user['id'] }}"
                                                class="list-group-item list-group-item-action cursor-pointer"
                                                wire:click="selectUser({{ $user['id'] }})" role="option"
                                                style="width: 320px;">
                                ...
                                <div class="col-12 col-md-6 mb-3 mb-md-0">
                                    ...
                                    <input class="form-control productInput" name="phone" type="text" wire:model="phone"
                            ...
                            <div class="row mt-2">
                                <div class="col-12">
                                    ...
                                    <textarea wire:model="address" name="address" class="form-control" id="exampleFormControlTextarea1" rows="5" spellcheck="false"></textarea>
                                </div>
                            </div>
                            <button type="button" class="btn btn-danger mt-3" wire:click="resetUser">
    ...
    <script>
        ...
        function arrowInit() {
            document.addEventListener('keydown', function(e) {
                ...
                // Stop browser's default up/down increment behavior
                e.preventDefault();
                ...
                } else if (e.key === 'ArrowUp' && rowIndex - 1 >= 0) {
        ...
        function addRow() {
            ...
            // setTimeout(() => selectElement.select2('open'), 0);
    ...
    <script>
        $('#invoiceForm').on('submit', function (e) {
            ...
            $('.invoice-row').each(function () {
                ...
                const qty = qtyValRaw === '' ? '' : parseFloat(qtyValRaw);
                const pieces = piecesValRaw === '' ? '' : parseFloat(piecesValRaw);
                ...
                if (qtyValRaw === '' && piecesValRaw === '') {
                    isValid = false;
                    rowValid = false;
                    qtyPiecesError.html('Enter Qty or Pieces.');
                }

                if (qtyValRaw !== '' && isNaN(qty)) {
                    isValid = false;
                    rowValid = false;
                    qtyPiecesError.html('Invalid Qty value');
                }

                if (piecesValRaw !== '' && isNaN(pieces)) {
                    isValid = false;
                    rowValid = false;
                    qtyPiecesError.html('Invalid Pieces value');
                }
            ...
            $.ajax({
                ...
                error: function () {
                    ...
                    Toast.fire({
                        icon: 'error',
                        title: 'An error occurred while saving the invoice.'
                    });
                }
            });
        });
    ...
    <script>
        $(document).ready(function() {
            $('#pending_invoice').click(function() {
                ...
                $.ajax({
                    url: '{{ route('admin.save-pending-invoice') }}',
                    type: 'POST',
                    data: formData,
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
