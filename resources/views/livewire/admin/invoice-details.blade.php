                        ...
                        <div class="card overflow-hidden">
                            <div class="card-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    <h3>{{ $page1 }}</h3>
                                    @if((Auth::user()->role == ADMIN_ROLE || Auth::user()->role == STAFF_ROLE) && $invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS)
                                        <button type="button" class="btn btn-danger" onclick="resetUser()">
                                            ...
                                            <tr class="invoice-row">
                                                <td style="width: 280px;">
                                                    <div class="position-relative">
                                                        <div>
                                                            ...
                                                            @if($invoiceDetails->status == ACTIVE_STATUS)
                                                                <input type="text"
                                                            ...
                                                            @else
                                                                <select id="companySelect{{$index}}"
                                                                        class="js-example-basic-single3 form-select {{isset($item->product) && !empty($item->product->name) ? 'pass':''}}"
                                                                        style="width: 100%;">
                                                                                                                                      <option value="">{{!empty($item->product) ? ($item->product->name.' '.$item->product->strength.' '.\Illuminate\Support\Str::limit($item->product->type,3,'') .' - '.\Illuminate\Support\Str::limit($item->product->company->name,3,'')) :'Product Deleted'}}</option>
                                                    ...
                                                    <!-- Hidden inputs -->
                                                    <input value="{{$item->id}}" name="rows[{{ $index }}][orderItemId]"
                                                           type="hidden"
                                                           class="form-control">
                                                    <input value="{{$item->company_id}}"
                                                           name="rows[{{ $index }}][companyId]" type="hidden"
                                                           class="form-control">
                                                     <input value="{{!empty($item->product) ?  $item->product->name :''}}"
                                                           name="rows[{{ $index }}][productName]" type="hidden"
                                                           class="form-control">
                                                    <input value="{{$item->product_id}}"
                                                           name="rows[{{ $index }}][product_id]" type="hidden"
                                                           class="form-control">
                                                    <input value="{{!empty($item->product) ? $item->product->box_per_pic :''}}"
                                                           name="rows[{{ $index }}][box_per_pic]" type="hidden"
                                                           class="form-control">
                                                    <input value="{{!empty($item->product) ? $item->product->unit_price : ''}}"
                                                           name="rows[{{ $index }}][actual_unit_price]" type="hidden"
                                                           class="form-control">
                                                    <input
                                                        value="{{empty($item->price) ? $item->product->unit_price : $item->price}}"
                                                        name="rows[{{ $index }}][unit_price]" type="hidden"
                                                        class="form-control">
                                                </td>
                                                ...
                                                <td>
                                                    <div class="d-flex flex-column">
                                                        <div class="d-flex">
                                                            <input type="number"
                                                                   {{$invoiceDetails->status == ACTIVE_STATUS ? 'readonly' :''}} name="rows[{{ $index }}][qty]"
                                                                   value="{{$item->box_qty ?? ''}}"
                                                                   class="form-control me-2 qty-field" placeholder="Box"
                                                                   style="width: 50%;">
                                                            <input type="number"
                                                                   {{$invoiceDetails->status == ACTIVE_STATUS ? 'readonly' :''}} step="any"
                                                                   name="rows[{{ $index }}][pieces]"
                                                                   value="{{$item->quantity ?? ''}}"
                                                                   class="form-control pieces-field"
                                                                   placeholder="Pieces"
                                                                   style="width: 50%;">
                                                        </div>
                                                    </div>
                                                ...
                                                <td>
                                                    <button type="button" onclick="removeRow(this)"
                                                            {{$invoiceDetails->status == ACTIVE_STATUS ? 'disabled' :''}}
                                                            class="btn btn-danger btn-xs"
                                                            @if (count($orderItems) === 1) disabled @endif>
                                            ...
                                            @if (auth()->user()->role != USER_ROLE)
                                                <td colspan="2">
                                                    @if($invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS)
                                                        <button onclick="addRow()" type="button"
                                                                class="btn btn-primary btn-xs mb-2" id="add-row-btn"><i
                                                                class="fa-solid fa-plus-circle"></i></button>
                                                    @endif
                                                    ...
                                                    <div
                                                        ...
                                                        <textarea id="note" name="note" wire:model="note"
                                                ...
                                                <div class="d-flex justify-content-end flex-column align-items-end">
                                                    ...
                                                    @can('apply other charge')
                                                        <div class="d-flex align-items-center gap-2 mb-2">
                                                            ...
                                                            <input wire:model="otherCharge" name="otherCharge"
                                                                   type="text" value="{{$otherCharge}}"
                                                                   step="any"
                                                                   {{$invoiceDetails->status == ACTIVE_STATUS ? 'readonly' :''}}
                                                    ...
                                                    <div class="d-flex align-items-center gap-2 mb-2">
                                                        ...
                                                        <input wire:model="grandTotal" type="text" name="grandTotal"
                                                               value="{{$grandTotal}}"
                                                               id="grandTotal"
                                                    ...
                                                    @can('apply invoice paid amount')
                                                        <div class="d-flex align-items-center gap-2 mb-2">
                                                            ...
                                                            <input wire:model="paidAmount" type="text" id="paidAmount"
                                                                   value="{{$paidAmount}}"
                                                                   name="paidAmount"
                                                                   {{$invoiceDetails->status == ACTIVE_STATUS ? 'readonly' :''}}
                                                    ...
                                                    @if (auth()->user()->role != USER_ROLE)
                                                        ...
                                                        <div class="d-flex align-items-center gap-2">
                                                            ...
                                                            <input wire:model="dueAmount" type="text" name="dueAmount"
                                                                   value="{{$dueAmount}}"
                                                                   class="form-control text-end" placeholder="0.00"
                                                    ...
                                                    @else
                                                        <input wire:model="dueAmount" type="hidden" name="dueAmount"
                                                               value="{{$dueAmount}}"
                                ...
                                @if (auth()->user()->role == ADMIN_ROLE)
                                    <div class="row">
                                        <div class="d-flex flex-wrap gap-2 mt-3">
                                            <!-- Submit Button -->
                                            @if($invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS)
                                                <button type="button" class="btn btn-primary submit add-new-row"
                                                        id="pending_invoice">
                                                    <span>Make It Pending</span>
                                                </button>
                                            @endif

                                            @if (auth()->user()->role == ADMIN_ROLE && $invoiceDetails->status == PENDING_STATUS || $invoiceDetails->status == DRAFT_STATUS)
                                                ...
                                                <button type="button" class="btn btn-warning submit"
                                                    ...
                                                    <span>Print Pending Invoice With Discount &nbsp; <i
                                                            class="fa fa-print"></i></span>
                                                </button>
                                            @endif
                                        </div>
...
@push('scripts')
    ...
    <script>
        ...
        function resetUser(){
            document.querySelector('[name="userName"]').value = "Unknown";
            document.querySelector('[name="phone"]').value = "Unknown";
            document.querySelector('[name="address"]').value = "Unknown";
        ...
        function customRoundFloat(value) {
            ...
            if (decimalPart >= 0.40) {
                return (Math.ceil(value)); // Must return string with .00
            } else {
        ...
        function calculateTotals() {
            ...
            const otherCharge = parseAmount(getInput("otherCharge")?.value) || 0;
            const paidAmount = parseAmount(getInput("paidAmount")?.value) || 0;
        ...
        $(document).ready(function() {
            $('#pending_invoice').click(function() {
                ...
                let dueAmountRaw = $('[name="dueAmount"]').val();
                let dueAmount = parseFloat(dueAmountRaw.replace(/,/g, ''));

                if (dueAmount < 0) {
                    $('.due-amount-error').text('Due amount cannot be negative.');
                    return;
                } else {
